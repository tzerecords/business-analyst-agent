...
<p align="center"><img src="assets/example/purchase-request.svg" width="640" alt="Purchase request, generated from skills/bpmn/examples/purchase-request.spec.json"></p>

Generated by the skill from skills/bpmn/examples/purchase-request.spec.json, then passed through the checker.
```bash
cd skills/bpmn && npm install                                 # once per checkout
...
```
**Without any coding tool.** Generator and checker are plain Node scripts, and neither one calls a language model. Run the worked example, then write your own spec against the format documented in `skills/bpmn/SKILL.md`.
```bash
git clone https://github.com/tzerecords/business-analyst-agent
